...
Step 3: 从抓取内容中识别 5-10 个核心功能，每个评估:
          - name (功能名)
...
Step 4: graph_write 每个功能为 FeatureNode（每个产品至少 5 个）。

CRITICAL:
- 必须 graph_query SourceInfo 优先；不要主动调 web_search/serper
- FeatureNode 必填字段：name (NOT feature_name), category, maturity, differentiation, product
- 每产品最少 5 个 FeatureNode
...
Step 3: 对每个产品识别:
          ...
          - plan list: [{name, price, billing_cycle, features[]}, ...]
          - value_score (0.0-1.0)
