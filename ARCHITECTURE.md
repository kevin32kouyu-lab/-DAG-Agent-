...
- `src/llm_gateway/gateway.py`：封装 Anthropic 和 OpenAI-compatible 调用；豆包 Ark EP 不传 `response_format`，避免不支持该参数的模型返回 400。
...
- 豆包 Ark EP 不支持 `response_format.type=json_object` 时，网关会自动省略该参数，继续依赖提示词和本地 JSON 解析兜底。
