...
function saveHistory(entries: HistoryTask[]) {
  localStorage.setItem('compagent_history', JSON.stringify(entries.slice(0, 20)));
}
    ...
    try {
      ...
      if (resp.ok) {
        ...
        const entry: HistoryTask = {
          ...
          targets: targets.join(', '), status: 'running', duration: '...',
        };
        const updated = [entry, ...history.filter(h => h.id !== data.task_id)];
        setHistory(updated);
        saveHistory(updated);
      ...
      // backend likely not running — navigate mock
      const mockId = `task_${targets.length}_${Date.now().toString(36)}`;
      setActiveTaskId(mockId);
      const entry: HistoryTask = { id: mockId, time: nowStr(), targets: targets.join(', '), status: 'running', duration: '...' };
      const updated = [entry, ...history.filter(h => h.id !== mockId)];
      setHistory(updated);
      saveHistory(updated);
      navigate(`/task/${mockId}/monitor`);
    }
    setLoading(false);
  };
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-8">
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-lg p-6 space-y-4">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <div>
            ...
            <select value={industry} onChange={e => setIndustry(e.target.value)}
              className="w-full px-2 py-1.5 bg-gray-950 border border-gray-700 rounded text-sm text-gray-300 focus:border-cyan-600 outline-none">
          ...
          <div>
            ...
            <div className="flex rounded border border-gray-700 overflow-hidden">
              <button onClick={() => setExecMode('auto')}
                className={`flex-1 px-2 py-1.5 text-xs transition-colors border-r border-gray-700 ${
        ...
        {/* Target input */}
        <div>
          <label className="text-xs text-gray-500 block mb-1">目标产品</label>
        ...
        {/* Error */}
        {error && (
          <div className="bg-red-900/10 border border-red-800/30 rounded p-2 text-xs text-red-400 font-mono">{error}</div>
        )}
                ...
                <tr key={task.id} className="border-b border-gray-800/50 hover:bg-gray-800/30">
                  ...
                  <td className="px-4 py-2.5 space-x-2">
                    <button onClick={() => { setActiveTaskId(task.id); navigate(`/task/${task.id}/report`); }}
                      className="text-cyan-400 hover:text-cyan-300 text-xs">查看报告</button>
                    <button onClick={() => { setActiveTaskId(task.id); navigate(`/task/${task.id}/trace`); }}
                      className="text-gray-500 hover:text-gray-300 text-xs">溯源</button>
                    {task.status === 'running' && (
                      <button onClick={() => { setActiveTaskId(task.id); navigate(`/task/${task.id}/monitor`); }}
                        className="text-amber-400 hover:text-amber-300 text-xs">实时监控</button>
                    )}
                    {task.status === 'failed' && (
                      <button onClick={() => { setActiveTaskId(task.id); startAnalysis(); }}
                        className="text-red-400 hover:text-red-300 text-xs">重新运行</button>
                    )}
                    {task.status === 'completed' && (
                      <button onClick={() => { setActiveTaskId(task.id); navigate(`/task/${task.id}/monitor`); }}
                        className="text-gray-500 hover:text-gray-300 text-xs">查看日志</button>
                    )}
                  </td>
                </tr>
              ))}
